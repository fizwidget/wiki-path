...
var _fizwidget$wiki_path$Common_PriorityQueue_Model$removeHighestPrioritiesInternal = F3(
	function (priorityQueue, howMany, values) {
		removeHighestPrioritiesInternal:
		while (true) {
			if (_elm_lang$core$Native_Utils.cmp(howMany, 0) > 0) {
				...
				var _v5 = updatedPriorityQueue,
					_v6 = howMany - 1,
					_v7 = {ctor: '::', _0: value, _1: values};
				priorityQueue = _v5;
				howMany = _v6;
				values = _v7;
				continue removeHighestPrioritiesInternal;
			} else {
				return {ctor: '_Tuple2', _0: values, _1: priorityQueue};
			}
		}
	});
var _fizwidget$wiki_path$Common_PriorityQueue_Model$removeHighestPriorities = F2(
	function (priorityQueue, howMany) {
		return A2(
			...
			_elm_lang$core$List$filterMap(_elm_lang$core$Basics$identity),
			A3(
				_fizwidget$wiki_path$Common_PriorityQueue_Model$removeHighestPrioritiesInternal,
				priorityQueue,
				howMany,
				{ctor: '[]'}));
	});
...
var _fizwidget$wiki_path$Pathfinding_Update$maxInFlightRequests = 4;
...
var _fizwidget$wiki_path$Pathfinding_Update$followPaths = F2(
	function (model, pathsToFollow) {
		var updatedModel = _elm_lang$core$Native_Utils.update(
			model,
			{
				inFlightRequests: model.inFlightRequests + _elm_lang$core$List$length(pathsToFollow)
			});
		var maybePathToDestination = _elm_lang$core$List$head(
			A2(
				_elm_lang$core$List$sortBy,
				function (pathToFollow) {
					return _elm_lang$core$List$length(pathToFollow.visited);
				},
				A2(
					_elm_lang$core$List$filter,
					function (pathToFollow) {
						return A2(_fizwidget$wiki_path$Pathfinding_Update$hasReachedDestination, pathToFollow.next, updatedModel.destination);
					},
					pathsToFollow)));
		var _p3 = maybePathToDestination;
		if (_p3.ctor === 'Just') {
			return A2(_fizwidget$wiki_path$Pathfinding_Update$destinationReached, updatedModel, _p3._0);
		} else {
			return {
				ctor: '_Tuple2',
				_0: _fizwidget$wiki_path$Model$Pathfinding(updatedModel),
				_1: _elm_lang$core$Platform_Cmd$batch(
					A2(_elm_lang$core$List$map, _fizwidget$wiki_path$Pathfinding_Update$fetchNextArticle, pathsToFollow))
			};
		}
	});
var _fizwidget$wiki_path$Pathfinding_Update$followHighestPriorityPaths = function (model) {
	var pathsToFollowCount = A2(_elm_lang$core$Basics$min, 2, _fizwidget$wiki_path$Pathfinding_Update$maxInFlightRequests - model.inFlightRequests);
	var _p4 = A2(_fizwidget$wiki_path$Common_PriorityQueue_Model$removeHighestPriorities, model.priorityQueue, pathsToFollowCount);
	var highestPriorityPaths = _p4._0;
	var updatedPriorityQueue = _p4._1;
	var updatedModel = _elm_lang$core$Native_Utils.update(
		model,
		{priorityQueue: updatedPriorityQueue});
	return (_elm_lang$core$List$isEmpty(highestPriorityPaths) && _elm_lang$core$Native_Utils.eq(updatedModel.inFlightRequests, 0)) ? _fizwidget$wiki_path$Pathfinding_Update$pathNotFound(updatedModel) : A2(_fizwidget$wiki_path$Pathfinding_Update$followPaths, updatedModel, highestPriorityPaths);
};
...
var _fizwidget$wiki_path$Pathfinding_Update$updateWithResult = F3(
	function (model, pathSoFar, articleResult) {
		...
		var _p5 = articleResult;
		if (_p5.ctor === 'Ok') {
			var _p6 = _p5._0;
			return A2(_fizwidget$wiki_path$Pathfinding_Update$hasReachedDestination, _p6.title, updatedModel.destination) ? A2(_fizwidget$wiki_path$Pathfinding_Update$destinationReached, updatedModel, pathSoFar) : A3(_fizwidget$wiki_path$Pathfinding_Update$updateWithArticle, updatedModel, pathSoFar, _p6);
		} else {
			return A2(_fizwidget$wiki_path$Pathfinding_Update$updateWithError, updatedModel, _p5._0);
		}
	});
var _fizwidget$wiki_path$Pathfinding_Update$update = F2(
	function (message, model) {
		var _p7 = message;
		if (_p7.ctor === 'FetchArticleResponse') {
			return A3(_fizwidget$wiki_path$Pathfinding_Update$updateWithResult, model, _p7._0, _p7._1);
		} else {
			return _fizwidget$wiki_path$Setup_Init$init;
		}
	});
